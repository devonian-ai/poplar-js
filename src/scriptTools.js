...
import {getCID} from './cidtools.js'
import {generateRandomString, processString} from './helpers.js'
...
async function makeScriptObject( {scriptInfo, increment, branch, previous}) {
    ...
    let previousVersion = {
        ...
        cid: null,
        version: {major: 0, minor: 0, patch: 0, branch: 'main', previous: []},
        provenance: [],
        local_path: null,
        increment: 'minor'
    };
    ...
    if (newVersion.local_path && newVersion.script_content.git_info.repo_name){
        newVersion.script_content.script_path = findRelativePath(
            newVersion.local_path, 
            scriptInfo.script_content.git_info.repo_name);
        }
    ...
    if (newVersion.script_content.name && !newVersion.script_content.script_id){
        newVersion.script_content.script_id = '#sc_' + processString(newVersion.script_content.name) + '_' + generateRandomString(8);
    }

    // Figure out the new version
    if (!branch) { branch = 'main' };
    if (!previous){increment = 'major'};
    if (increment){
        newVersion.version = updateVersion(
            newVersion.version, 
            increment, branch, [previous]);
    }
    ...
    newVersion.cid = await getCID(newVersion.script_content_string);
...
export {makeScriptObject, replaceFields, findRelativePath, getFileName};